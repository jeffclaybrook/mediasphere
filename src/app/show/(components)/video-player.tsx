...
import { CaptionsOff, CaptionsOn, Fullscreen, NextChapter, Pause, Play, PlayerSettings, Volume, VolumeOff } from "@/components/global/icons"
...
export default function VideoPlayer({ src, captions, chapters = [] }: PlayerProps) {
 ...
 useEffect(() => {
  ...
  const handleTimeUpdate = () => {
   ...
   const active = chapters
    .slice()
    .reverse()
    .find((chapter) => current >= chapter.time)
 ...
 const skipToNextChapter = () => {
  ...
  const current = video.currentTime
  const next = chapters.find(chapter => chapter.time > current)
   ...
   <div className="absolute bottom-0 w-full flex flex-col opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto">
    <div className="px-2">
     ...
     >
      ...
      <div className="absolute top-1/2 transform -translate-y-1/2 h-1 bg-blue-500 rounded-full transition-all duration-100 ease-linear" style={{ width: `${(progress / duration) * 100 || 0}%` }} />
      {chapters.map((chapter, i) => {
       const left = (chapter.time / duration) * 100
       return (
        <span
         key={i}
         title={chapter.title}
      ...
      <div className="absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-4 h-4 bg-white border border-blue-500 rounded-full shadow" style={{ left: `${(progress / duration) * 100 || 0}%` }} />
     </div>
    </div>
